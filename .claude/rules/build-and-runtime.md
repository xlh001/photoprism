...
- **Production images are the `-slim` develop image plus the built binary — read the `FROM` chain before concluding a package is absent.** Every current production Dockerfile (`docker/photoprism/resolute`, `plus/docker/plus`, `pro/docker/pro`, `portal/docker`) is a two-stage build: compile in `photoprism/develop:<codename>`, then `FROM photoprism/develop:<codename>-slim` for the runtime stage. Anything installed in a `-slim` image therefore ships to users, and shared runtime config belongs there rather than being repeated per edition. Grepping only the production Dockerfile will tell you a package is missing when it is actually inherited. `docker/photoprism/*` also contains legacy variants (see `docker/README.md`); the `Makefile` build targets are authoritative for what is actually published — resolute carries the unsuffixed `ce`/`preview-ce` tags, other codenames get suffixed ones and several are never built. A changed `-slim` base must be rebuilt and published **before** the next edition build, or those images are produced without whatever the base now provides.
- **AppArmor profiles cannot be shipped inside an image.** Policy is a host-kernel object; a profile baked into an image is inert unless the operator loads it and passes `security_opt: apparmor=<name>`, so shipping one is never a fix on its own. How the media delegates are installed in the base images is deliberate — check the related specs before changing it.
- **Verify `scripts/dist/convert/policy.xml` rules by probing ImageMagick, never by reading them.** Patterns match the names ImageMagick registers (`magick -list format` / `-list delegate`), not format names, and an unmatched pattern is silently ignored while still printing in `magick -list policy` — so an inert rule looks exactly like an enforced one. Which coders must stay enabled, how the cache limits track the resolution options, and which settings break conversion are documented in the related specs, along with the tooling that performs the probe against a built image.
